...
import { Cell, Slice, Message, Dictionary  } from '@ton/core';
...
export function computeExternalMessageFees(msgPrices: MsgPrices, cell: Cell) {
    ...
    // Collect stats
    const storageStats = collectCellStats(cell, true);
...
export function computeMessageForwardFees(msgPrices: MsgPrices, msg: Message) {
    ...
    // Body
    const bc = collectCellStats(msg.body, skipRef);
    storageStats.bits  += bc.bits;
    storageStats.cells += bc.cells;

    // NOTE: Extra currencies are ignored for now
    const fees = computeFwdFees(msgPrices, BigInt(storageStats.cells), BigInt(storageStats.bits));
    const res  = (fees * msgPrices.firstFrac) >> BigInt(16);
    const remaining = fees - res;
